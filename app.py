...
def init_db():
    """Otomatis membuat tabel jika belum ada, tidak perlu manual di Neon"""
    try:
...
init_db() # Jalankan saat aplikasi dimuat
...
# --- BAGIAN 2: GENERATE AI (SMARTER PROMPT) ---
if st.button("Generate Pasal AI", type="primary"):
    with st.spinner("Gemini sedang memikirkan pasal-pasal..."):
        ...
        try:
            ...
            st.error(f"Gagal memproses AI atau format JSON tidak valid. Error: {e}")

# --- BAGIAN 3: EDITOR PER KOTAK & PENYIMPANAN ---
    ...
    with col_save:
        if st.button("Simpan ke Database"):
            try:
                ...
                cur.close()
                conn.close()
                st.success("Tersimpan ke database Neon!")
            except Exception as e:
